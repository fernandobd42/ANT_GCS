...
@WebServlet("/StudentConsultAllController")
public class StudentConsultAllController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		students = ia.consultAll();
			...
			"margin-left: -4%; box-shadow: 10px 10px 10px rgba(0, 0, 0, 0.5); padding: 20px 20px 20px 20px;}</style>");
			...
			for (Student s : students) {
				buf.append("<tr>");
				buf.append("<td>"+s.getName()+"</td><td>"+s.getCpf()+"</td><td>"+s.getRa()+"</td><td>"+s.getEmail()+"</td><td>"+
				s.getSex()+"</td><td>"+s.getCourse()+"</td><td>"+s.getLattes()+"</td><td>"+s.getLinkedin()+"</td><td>"+
				s.getStreet()+"</td><td>"+s.getNumber()+"</td><td>"+s.getNeighborhood()+"</td><td>"+s.getCity()+"</td><td>"+
				s.getState()+"</td><td>"+s.getPhone()+"</td></tr></form></div>");	
			}
